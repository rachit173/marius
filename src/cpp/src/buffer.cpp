...
Partition* Partition::ConvertToPartition(const torch::Tensor &tensor) {
	...
	Partition* out_part = new Partition(partition_id, partition_size, embedding_size, torch::kFloat32, idx_offset, file_offset);
	return out_part;
}

...
class WorkerNode {
  ...
  // public:
    ...
    void RequestPartitions() {
      std::cout << " Started RequestPartitions thread " << std::endl;
      while (1) {
        // if ()
        int size;
        {
    ...
    void DispatchPartitionsToCoordinator() {
      std::cout << " Started DispatchPartitionsToCoordinator thread " << std::endl;
      while (1) {
        while (1) {
    ...
    void TransferPartitionsFromWorkerNodes() {
      std::cout << " Started TransferPartitionsFromWorkerNodes thread " << std::endl;
      while (1) {
        {
          while (1) {
            WriteLock w_lock(transfer_receive_parts_rw_mutex_);
    ...
    void TransferPartitionsToWorkerNodes() {
      std::cout << " Started TransferPartitionsToWorkerNodes thread " << std::endl;
...
/* Flow:
1. Program starts: worker processes and 1 co-ordinator process starts
Co-ordinator fills up what all partitions(metadata) are available
2, Worker process requests for # of partitions equal to its buffer size(initially) from the co-ordinator
3. Co-ordinator checks what all partitions are available and are not present on other nodes and sends metadata to worker for such partitions
4. Worker goes through co-ordinator's response and sends requests to the worker node where the partition is located. If partition is not located anywhere, it gets the partition from disk

5. Loop:
a. When worker finishes processing a partition, it registers the partition with the co-ordinator as available
b. Then it requests for a partition from the co-ordinator and waits for the metadata
c. If no other partition available --> WAIT
d. Else: get the partition data from the specified worker node
e. Parallely: if it can send the non-required partition to some other node, send it
f. If cannot send it --> store non-required partition to disk
*/